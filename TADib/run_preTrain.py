...
def get_score(test_result, val_result):
    ...
    print(folder_path)
    np.savetxt(
        f'{folder_path}/NewDataBiEvenSeqMaskSwat/{args.variate_index}dim/bestF1_{args.dataset}_{args.model_type}2Stack_{args.variate_index}dimNum{args.variate_index}_group{args.groups}_lradj{args.lradj}_{args.slide_win}_h{args.hidden_size}_bt{args.batch}_p{args.pred_len}_pred_label.csv',
        top1_best_info[-1], delimiter=",")
...
def get_save_path(feature_name=''):
    ...
    paths = [ #args.model_type
        f'{dir_path}/NewDataBiEvenSeqMaskSwat/{args.variate_index}dim/{args.dataset}_{args.model_type}_Pretrain2Stack_{args.variate_index}dimNew_group{args.groups}_lradj{args.lradj}_{args.slide_win}_h{args.hidden_size}_bt{args.batch}_p{args.pred_len}_{datestr}.pt',
...
if __name__ == "__main__":
    ...
    # index = [2, 4, 8, 10, 13, 14, 22, 24, 27, 28]
    # index = [1, 3, 7, 9, 12, 13, 21, 23, 26, 27]
    # index = [1, 3,  13, 21, 23]
    index = [1,   13,  23]
    index = 0
    ...
    cfg = {
        ...
        'test_slide_win': int(args.slide_win),
        'test_slide_stride': int(args.slide_win),
    }
    ...
    if args.train:
        ...
        model_load_path = f'{dir_path}/NewDataBiEvenSeqMaskSwat/{args.dataset}_{args.model_type}_Pretrain2Stack_lradj{args.lradj}_' \
